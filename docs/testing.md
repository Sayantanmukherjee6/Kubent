...
|---|---|---|---|
| **Unit** | `tests/unit/` | Config loading, provider factory, log generator output, settings validation, watcher components (detector, context builder, dedup tracker) | No |
| **Integration** | `tests/integration/` | Log source lifecycle (start/stop/stream), mock HTTP server for LLM pipeline, multi-service watcher scenarios, full watcher flow | Mocked only |
...
- **Watcher (Unit)**: Detector rule matching (32 rules), context builder window extraction, dedup tracker fingerprinting and TTL expiry, watcher orchestrator flow
